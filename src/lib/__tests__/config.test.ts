...
describe('loadConfig', () => {
  ...
  it('returns all-enabled defaults when no config file exists', () => {
    ...
    expect(config.tools).toEqual({ kubectl: true, listContexts: true, listNamespaces: true, helmRelease: true, prometheusQuery: false, awsCli: false, trivyScan: false, kubecostQuery: false, lokiQuery: false, jaegerQuery: false });
  });
