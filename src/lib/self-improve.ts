...
/**
 * Map a single assertion failure string to a human-readable suggestion.
 * Returns an empty string when the failure prefix is recognised but the inner
 * pattern does not match (e.g. a `Severity:` line without the expected/got format).
 */
function classifySuggestion(failure: string, scenario: string): string {
  ...
  if (failure.startsWith('Missing expected keyword:')) {
    const kw = extractQuotedKeyword(failure);
    if (!kw) return '';
    return (
  ...
  if (failure.startsWith('Found forbidden keyword:')) {
    const kw = extractQuotedKeyword(failure);
    if (!kw) return '';
    return (
  ...
  if (failure.startsWith('Agent error:')) {
    return (
 ...
 */
export function generateSuggestion(scenario: string, _prompt: string, failures: string[]): string {
  if (failures.length === 0) return 'No actionable suggestion generated.';
  return failures.map((f) => classifySuggestion(f, scenario)).filter(Boolean).join(' | ');
}
