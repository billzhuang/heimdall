...
- Use \`kubectl get -o wide\` for nodes and pods; \`kubectl rollout status --timeout=5s\` to detect stuck rollouts; \`kubectl get events --sort-by='.lastTimestamp'\` for recent warnings.
