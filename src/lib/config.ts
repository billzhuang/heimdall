...
// Single source of truth for each tool's default enabled state — used both as
// the per-field fallback and as the whole-block fallback below, so the two
...
const ToolsSchema = v.nullish(
  v.object({
    kubectl: v.nullish(v.boolean(), TOOLS_DEFAULTS.kubectl),
    listContexts: v.nullish(v.boolean(), TOOLS_DEFAULTS.listContexts),
    listNamespaces: v.nullish(v.boolean(), TOOLS_DEFAULTS.listNamespaces),
    helmRelease: v.nullish(v.boolean(), TOOLS_DEFAULTS.helmRelease),
    prometheusQuery: v.nullish(v.boolean(), TOOLS_DEFAULTS.prometheusQuery),
    awsCli: v.nullish(v.boolean(), TOOLS_DEFAULTS.awsCli),
    trivyScan: v.nullish(v.boolean(), TOOLS_DEFAULTS.trivyScan),
    kubecostQuery: v.nullish(v.boolean(), TOOLS_DEFAULTS.kubecostQuery),
    lokiQuery: v.nullish(v.boolean(), TOOLS_DEFAULTS.lokiQuery),
    jaegerQuery: v.nullish(v.boolean(), TOOLS_DEFAULTS.jaegerQuery),
    datadogQuery: v.nullish(v.boolean(), TOOLS_DEFAULTS.datadogQuery),
    newRelicQuery: v.nullish(v.boolean(), TOOLS_DEFAULTS.newRelicQuery),
    cdkQuery: v.nullish(v.boolean(), TOOLS_DEFAULTS.cdkQuery),
  }),
  TOOLS_DEFAULTS,
);
